#'@title SFS similarity measure simSGFDK8
#'@description SFS similarity measure values using simSGFDK8 computation technique with membership,non-membership, indeterminacy membership, and refusal membership values of two objects or set of objects.
...
#'@export
simSGFDK8<-function(ma,na,mb,nb,ia,ib,ra,rb,k){
  c<-matrix(0,nrow=nrow(ma),ncol=ncol(ma))
  for (i in 1:nrow(ma)) {
    for(j in 1:ncol(ma))
      c[i,j]<-sqrt((ma[i,j]^2*mb[k,j]^2)+(na[i,j]^2*nb[k,j]^2)+(ia[i,j]^2*ib[k,j]^2)+(ra[i,j]^2*ra[k,j])^2)/(((ma[i,j]^4)+(na[i,j]^4)+(ia[i,j]^4)+(ra[i,j]^4))^(1/4)*(((mb[k,j]^4)+(nb[k,j]^4)+(ib[k,j]^4)+(ra[k,j]^4))^(1/4)))
  }
  for(j in 1:ncol(c)){
