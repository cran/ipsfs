#'@title PFS similarity measure simZHFLL2
#'@description PFS similarity measure values using simZHFLL2 computation technique with membership, and non-membership values of two objects or set of objects.
...
#'@export
simZHFLL2<-function(ma,na,mb,nb,k){
  c<-matrix(0,nrow=nrow(ma),ncol=ncol(ma))
  for (i in 1:nrow(ma)) {
    for(j in 1:ncol(ma))
      c[i,j]<-(2^(1-((1/2)*(abs(ma[i,j]^2-mb[k,j]^2)+abs(na[i,j]^2-nb[k,j]^2)))))-1
  }
  for(j in 1:ncol(c)){
