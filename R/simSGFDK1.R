#'@title SFS similarity measure simSGFDK1
#'@description SFS similarity measure values using simSGFDK1 computation technique with membership,non-membership, and indeterminacy membership values of two objects or set of objects.
...
#'@export
simSGFDK1<-function(ma,na,mb,nb,ia,ib,k){
  c<-matrix(0,nrow=nrow(ma),ncol=ncol(ma))
  for (i in 1:nrow(ma)) {
    for(j in 1:ncol(ma))
      c[i,j]<-((ma[i,j]^2)*(mb[k,j]^2)+(na[i,j]^2)*(nb[k,j]^2)+(ia[i,j]^2)*(ib[k,j]^2))/((ma[i,j]^4+na[i,j]^4+ia[i,j]^4)+(mb[k,j]^4+nb[k,j]^4+ib[k,j]^4)-((ma[i,j]^2)*(mb[k,j]^2)+(na[i,j]^2)*(nb[k,j]^2)+(ia[i,j]^2)*(ib[k,j]^2)))
  }
  for(j in 1:ncol(c)){
