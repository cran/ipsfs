#'@title PFS similarity measure simPYY3
#'@description PFS similarity measure values using simPYY3 computation technique with membership, and non-membership values of two objects or set of objects.
...
#'@export
simPYY3<-function(ma,na,mb,nb,k){
  c<-matrix(0,nrow=nrow(ma),ncol=ncol(ma))
  for (i in 1:nrow(ma)) {
    for(j in 1:ncol(ma))
      c[i,j]<-(min(ma[i,j]^2,mb[k,j]^2)+min((1-na[i,j]^2),(1-nb[k,j]^2)))/(max(ma[i,j]^2,mb[k,j]^2)+max((1-na[i,j]^2),(1-nb[k,j]^2)))
  }
  for(j in 1:ncol(c)){
