#'@title SFS similarity measure simSGFDK2
#'@description SFS similarity measure values using simSGFDK2 computation technique with membership,non-membership, indeterminacy membership, and refusal membership values of two objects or set of objects.
...
#'@export
simSGFDK2<-function(ma,na,mb,nb,ia,ib,ra,rb,k){
  c<-matrix(0,nrow=nrow(ma),ncol=ncol(ma))
  for (i in 1:nrow(ma)) {
    for(j in 1:ncol(ma))
      c[i,j]<-((ma[i,j]^2)*(mb[k,j]^2)+(na[i,j]^2)*(nb[k,j]^2)+(ia[i,j]^2)*(ib[k,j]^2)+(ra[i,j]^2)*(rb[k,j]^2))/((ma[i,j]^4+na[i,j]^4+ia[i,j]^4+ra[i,j]^4)+(mb[k,j]^4+nb[k,j]^4+ib[k,j]^4+rb[k,j]^4)-((ma[i,j]^2)*(mb[k,j]^2)+(na[i,j]^2)*(nb[k,j]^2)+(ia[i,j]^2)*(ib[k,j]^2)+(ra[i,j]^2)*(rb[k,j]^2)))
  }
  for(j in 1:ncol(c)){
