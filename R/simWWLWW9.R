#'@title SFS similarity measure simWWLWW9
#'@description SFS similarity measure values using simWWLWW9 computation technique with membership,non-membership, indeterminacy membership, and refusal membership values of two objects or set of objects.
...
#'@export
simWWLWW9<-function(ma,na,mb,nb,ia,ib,ra,rb,k){
  c<-matrix(0,nrow=nrow(ma),ncol=ncol(ma))
  for (i in 1:nrow(ma)) {
    for(j in 1:ncol(ma))
      c[i,j]<-tan((pi/4)+((pi/4))*max(abs(ma[i,j]^2-mb[k,j]^2),abs(na[i,j]^2-nb[k,j]^2),abs(ia[i,j]^2-ib[k,j]^2),abs(ra[i,j]^2-rb[k,j]^2)))
  }
  for(j in 1:ncol(c)){
