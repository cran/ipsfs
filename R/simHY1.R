#'@title IFS similarity measure simHY1
#'@description IFS similarity measure values using simHY1 computation technique with membership, and non-membership values of two objects or set of objects.
...
#'@export
simHY1<-function(ma,na,mb,nb,k){
  c<-matrix(0,nrow=nrow(ma),ncol=ncol(ma))
  for (i in 1:nrow(ma)) {
    for(j in 1:ncol(ma))
      c[i,j]<-((min(ma[i,j],mb[k,j])+min(na[i,j],nb[k,j]))/(max(ma[i,j],mb[k,j])+max(na[i,j],nb[k,j])))
  }
  for(j in 1:ncol(c)){
