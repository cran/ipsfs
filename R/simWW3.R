#'@title PFS similarity measure simWW3
#'@description PFS similarity measure values using simWW3 computation technique with membership, and non-membership values of two objects or set of objects.
...
#'@export
simWW3<-function(ma,na,mb,nb,k){
  c<-matrix(0,nrow=nrow(ma),ncol=ncol(ma))
  for (i in 1:nrow(ma)) {
    for(j in 1:ncol(ma))
      c[i,j]<-cos((pi/2)*max(abs(ma[i,j]^2-mb[k,j]^2),abs(na[i,j]^2-nb[k,j]^2)))
  }
  for(j in 1:ncol(c)){
