#'@title PFS similarity measure simPG1
#'@description PFS similarity measure values using simPG1 computation technique with membership, and non-membership values of two objects or set of objects.
...
#'@export
simPG1<-function(ma,na,mb,nb,p,l,t,k){
  c<-matrix(0,nrow=nrow(ma),ncol=ncol(ma))
  for (i in 1:nrow(ma)) {
    for(j in 1:ncol(ma))
      c[i,j]<-(abs(((l-1)*(ma[i,j]^2-mb[k,j]^2))-(na[i,j]^2-nb[k,j]^2))^p)+(abs(((l-t)*(na[i,j]^2-nb[k,j]^2))-(t*(ma[i,j]^2-mb[k,j]^2)))^p)
  }
  for(j in 1:ncol(c)){
    sum<-1-((rowSums(c)/(2*ncol(c)*((l)^p)))^(1/p))
  }
  sum
}
